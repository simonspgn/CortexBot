...
class Dispatcher:
    ...
    def analyzers(self,input_args):
        ...
        # If a more specific search is required (more arguments given by the user).
            ...
            # Case 2: User wants to find a specific analyzer
                ...
                analyzer = cortex_object.analyzerName(input_args[1])
    ...
    def jobs(self,input_args):
        ...
        # If a more specific search is required (more arguments given by the user).
            ...
            if input_args[1] in analyzers_ls:
                return job_object.formatJobAnalyzerName(input_args[1],jobs_ls_all)
            ...
            # Third case: Job search by ID.
                ...
                try:
                    job = cortex_object.jobId(input_args[1])
                    # Create the response
        ...
        # If 3 input arguments are given, the user requests more specific information concerning a given job: Report or Artifacts
            ...
            if second_arg=="report":
                ...
                try:
                    job = cortex_object.jobId(input_args[2])
            ...
            # User is looking for artifacts
                ...
                artifact = cortex_object.jobArtifacts(input_args[2])
                if artifact==[]:
    ...
    def run(self, input_args, query_user):
        ...
        # Set the force flag
        input_args, flag = run_object.forceFlag(input_args)
        # Check first argument corresponds to a valid enabled analyzer.
        if input_args[1] in analyzers_ls and length<=5:
            analyzer_name = input_args[1]
            try:
                # Check if all the arguments given are valid.
                if type(run_object.getArgs(input_args))==str:
                    return run_object.getArgs(input_args)
                # If they are valid, set the arguments variables.
                else:
                    (data_type_query,data,tlp)=run_object.getArgs(input_args)
        ...
        elif input_args[1] == "*" and length<=5:
            try:
                # Check if all the arguments given are valid.
                if type(run_object.getArgs(input_args))==str:
                    return run_object.getArgs(input_args)
    ...
    def main(self):
        ...
        if not self.valid(query_token):
            ...
            first_arg = input_args[0].lower()
            if first_arg=="analyzers":
                response = self.analyzers(input_args)
            elif first_arg=="jobs":
                response = self.jobs(input_args)
            elif first_arg=="run" and len(input_args)>1:
                response = self.run(input_args,query_user)
            elif first_arg=="help" and len(input_args)==1:
                response = help_object.mainHelp(link)
            elif first_arg=="help" and input_args[1].lower()=="run" and len(input_args)==2:
...
if __name__ == "__main__":
    dispatcher_object = Dispatcher()
    print(dispatcher_object.main())
